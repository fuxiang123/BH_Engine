...
namespace BH_Engine
{
    public class BulletBehaviour : MonoBehaviour
    {
        ...
        // 子弹上附加的发射器
        public List<GameObject> emitters = new();
        ...
        {
            var bulletFinalConfig = bulletConfig.GetFinalConfig(currentTime);
            currentTime += Time.fixedDeltaTime;
            var prePosition = transform.position;
            if (BulletMoveScript?.Count > 0)
            {
                for (var i = 0; i < BulletMoveScript.Count; i++)
            ...
            if (emitters.Count > 0)
                for (var i = 0; i < emitters.Count; i++)
                {
                    ...
                    var emitterConfigAngle =
                        emitters[i].GetComponent<BaseEmitter>()?.EmitterConfig?.emitterAngle?.value ?? 0;
            ...
            if (currentTime >= bulletFinalConfig.lifeTime ||
                (bulletFinalConfig.maxDistance > 0 && realDistance >= bulletFinalConfig.maxDistance))
        ...
        public void Init(BulletConfig bulletConfig, Vector3 direction, Action<GameObject> OnReleaseBullet)
        {
            ...
            if (bulletConfig.BulletMoveScript != null && bulletConfig.BulletMoveScript.Count > 0)
                for (var i = 0; i < bulletConfig.BulletMoveScript.Count; i++)
            ...
            if (bulletConfig.emitterProfile != null && bulletConfig.emitterProfile.Count > 0)
            {
                ...
                for (var i = 0; i < configEmitterProfiles.Count; i++)
                {
                    var profileEmitter = emitters[i].GetComponent<ProfileEmitter>();
